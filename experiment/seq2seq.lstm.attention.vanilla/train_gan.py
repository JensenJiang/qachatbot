...
def main():
    ...
    with tf.Session() as sess:
        model = GANBasicModel(config, 'train')
        ...
        while clock.cur_epoch < config.max_epoch:
            ...
            for minibatch in dataset.get_dataset_iter():
                ...
                if(loop%Config.critic==0):
                    fetch_dict = model.build_fetch_dict(['d_loss', 'g_loss','dis_updates','d_clip'])
                else:
                    fetch_dict = model.build_fetch_dict(['d_loss','g_loss','gen_updates'])
                outputs = sess.run(fetch_dict, feed_dict=feed_dict)
                print('d_loss: {} g_loss:{}'.format(outputs['d_loss','g_loss']))
